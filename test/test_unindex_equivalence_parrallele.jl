...
# (canonical_suit_profiles and are_hands_isomorphic remain unchanged)
...
@testset "Direct Hand Isomorphism Equivalence Tests" begin
    ...
    for (rounds, cards_per_round, name) in test_configs
        @testset "$name Testing" begin
            ...
            # -------------------------------------------------------------------
            # 1. Allocate thread-local buffers (1 per thread)
            # -------------------------------------------------------------------
            n_threads = Threads.nthreads()
            tl_c_bufs = [zeros(UInt8, total_cards) for _ in 1:n_threads]
            tl_jl_bufs = [zeros(UInt8, total_cards) for _ in 1:n_threads]
            tl_scratches = [HandIsomorphism.HandUnindexState() for _ in 1:n_threads]

            # 1. Setup & Error collector
            # -------------------------------------------------------------------
            failed_indices = Vector{UInt64}()
            failed_lock = ReentrantLock()

            # Divide indices into chunks (one chunk per available thread)
            n_chunks = Threads.nthreads()
            # Stride partition across indices
            chunks = [test_indices[k:n_chunks:end] for k in 1:n_chunks]

            # -------------------------------------------------------------------
            # 2. Parallel Processing with Task-Local Buffers
            # -------------------------------------------------------------------
            @sync for chunk in chunks
                Threads.@spawn begin
                    # Each task allocates its own buffers once:
                    c_buf = zeros(UInt8, total_cards)
                    jl_buf = zeros(UInt8, total_cards)
                    scratch = HandIsomorphism.HandUnindexState()
            ...
            # -------------------------------------------------------------------
            # 3. Single assertion for all indices
            # -------------------------------------------------------------------
            @test isempty(failed_indices)
            if !isempty(failed_indices)
                @error "Failed indices (first 10): $(failed_indices[1:min(end, 10)])"
            end
